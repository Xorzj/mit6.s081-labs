...
// in order to clone user pages to kernel pages
int ukvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start,
                     uint64 sz) {
  pte_t *pte;
  uint64 pa, i;
  // 扩容的时候需要 PGROUNDUP 对齐
  for (i = PGROUNDUP(start); i < start + sz; i += PGSIZE) {
    if ((pte = walk(src, i, 0)) == 0) {
      panic("kvmcopymappings : no exist pte");
    }
    if (!(*pte & (PTE_V))) {
      panic("kvmcopymappings : no vaild pte ");
    }
    pa = PTE2PA(*pte);
    if (mappages(dst, i, PGSIZE, pa, PTE_FLAGS(*pte) & (~PTE_U)) != 0) {
      goto err;
      // panic("kvmcopymappings : map dst pages error");
    }
  }
  return 0;
err:
...
void vm_print(pagetable_t pagetable, int level) {
  for (int i = 0; i < 512; i++) {
    ...
    if (pte & PTE_V) {
      ...
      printf("%d: pte %p pa %p\n", i, pte, PTE2PA(pte));
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0) vm_print((pagetable_t)PTE2PA(pte), level - 1);
    }
  }
}
void vmprint(pagetable_t pagetable) {
  printf("page table %p\n", pagetable);
  vm_print(pagetable, 2);
}