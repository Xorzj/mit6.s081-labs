...
pagetable_t proc_kpagetable(struct proc *p) {
  ...
  // ukvmmap(kpagetable, TRAPFRAME, PGSIZE, (uint64)(p->trapframe), PTE_R |
  // PTE_W);
...
void userinit(void) {
  ...
  ukvmcopymappings(p->pagetable, p->kpagetable, 0, PGSIZE);
  p->sz = PGSIZE;
...
int growproc(int n) {
  ...
  if (n > 0) {
    ...
    if (ukvmcopymappings(p->pagetable, p->kpagetable, sz, n) < 0) {
...
int fork(void) {
  ...
  if (ukvmcopymappings(np->pagetable, np->kpagetable, 0, p->sz) < 0) {
    freeproc(np);
    release(&np->lock);
    return -1;
  }
